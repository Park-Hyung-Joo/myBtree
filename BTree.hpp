...
template <typename _key, typename _value>
 _node<_key, _value>* Btree<_key, _value>::find_seat(pnode n,item *k){ // return leaf if no key or key is in leaf =>check if key is in leaf
    int i;
    if(isTerminal(n)) {
    	for(i=0;i<n->num;i++){
...
int Btree<_key, _value>::propagation(pnode getLeft, pnode getRight, pnode target, item* popped){//target is not leaf
    if (!target) { //child of target is root => we need new node for new root
